...
[ ! -d files/arm ] && ./scripts/get-plugins.sh

package() {
    mkdir ipk-build

    mkdir -p ipk-build/data/{etc/init.d,usr/sbin/uu}
    cp files/$1/uuplugin ipk-build/data/usr/sbin/uu
    cp files/$1/uu.conf ipk-build/data/etc
